import { useEffect, useState, useCallback } from "react";
import { getAnnouncements, deactivateAnnouncement } from "../../api/announcement.api";
import SearchBar from "../../components/shared/SearchBar";
import Pagination from "../../components/shared/Pagination";
import Button from "../../components/ui/Button";
import { usePagination } from "../../hooks/usePagination";

export default function AnnouncementList({ refreshKey }) {
  const [announcements, setAnnouncements] = useState([]);
  const [search, setSearch] = useState("");
  const [loading, setLoading] = useState(true);
  const { page, setPage, limit, pages, setTotal } = usePagination(10);

  const fetchAnnouncements = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await getAnnouncements({ page, limit, search });
      setAnnouncements(data.data);
      setTotal(data.pagination.total);
    } finally {
      setLoading(false);
    }
  }, [page, limit, search, setTotal]);

  useEffect(() => {
    fetchAnnouncements();
  }, [fetchAnnouncements, refreshKey]);

  const handleDeactivate = async (id) => {
    if (!confirm("Remove this announcement?")) return;
    await deactivateAnnouncement(id);
    fetchAnnouncements();
  };

  return (
    <div className="flex flex-col gap-4">
      <SearchBar
        value={search}
        onChange={(value) => {
          setSearch(value);
          setPage(1);
        }}
        placeholder="Search by title"
      />

      {loading ? (
        <p className="text-sm text-charcoal/60">Loading...</p>
      ) : (
        <div className="divide-y divide-rule border-y border-rule">
          {announcements.length === 0 && (
            <p className="py-8 text-center text-sm text-charcoal/50">
              No announcements yet — post your first one above.
            </p>
          )}
          {announcements.map((a) => (
            <div
              key={a._id}
              className="flex flex-wrap items-center justify-between gap-3 px-2 py-4 transition-colors hover:bg-ink/[0.02]"
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-ink">{a.title}</p>
                <p className="truncate text-xs text-charcoal/50">
                  To: {a.audience.join(", ")} · {a.classId ? a.classId.name : "School-wide"} · by{" "}
                  {a.authorId?.fullName}
                </p>
              </div>
              <Button size="sm" variant="dangerGhost" onClick={() => handleDeactivate(a._id)}>
                Remove
              </Button>
            </div>
          ))}
        </div>
      )}

      <Pagination page={page} pages={pages} onPageChange={setPage} />
    </div>
  );
}
